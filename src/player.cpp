...
uint32_t Player::getAttackSpeed() const
{
	...
	int32_t vocAs = vocation->getAttackSpeed();
	if (vocAs - (getSkillLevel(SKILL_AXE)*8) <= 400){
		vocAs = 400;
	}else{
		vocAs = (vocAs - (getSkillLevel(SKILL_AXE)*8));	
	}
	return std::max<uint32_t>(400, vocAs );
	
	//const Item* weapon = getWeapon(true);
	

	//int32_t weaponAs = weapon->getAttackSpeed();
	
	//if (!weapon || weaponAs == 0) {
		
	//}

	//return vocAs - (weaponAs + getSkillLevel(SKILL_AXE))*16;
}
