...
MonsterType* Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading /*= false*/)
{
	...
	if ((node = monsterNode.child("flags"))) {
		for (auto flagNode : node.children()) {
			...
			} else if (strcasecmp(attrName, "isSaga") == 0) {
				mType->info.isIsSaga = attr.as_bool();		
			...
			} else if (strcasecmp(attrName, "isSagaNumber") == 0) {
				int32_t isSagaNumber = pugi::cast<int32_t>(attr.value());
				mType->info.isSagaNumber = isSagaNumber;
